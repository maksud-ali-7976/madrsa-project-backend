...
app.onAfterHandle(({ logger, response, path }) => {
  logger.add("Response - ");

  try {
    // Skip streamed/native Response objects
    if (
      path.includes("swagger") ||
      response instanceof Response
    ) {
      logger.add("[Stream/Response skipped]");
    } else {
      logger.add(
        JSON.stringify(response, null, 2).substring(0, 150),
      );
    }
  } catch (error) {
    logger.add("[Unable to stringify response]");
  }
...
export const routeMap: Map<any, { modules: ModuleId[] }> =
  new Map();
...
for (let route of app.routes) {
  if (route.hooks.detail?.summary) {
    routeMap.set(
      route.path,
      JSON.parse(route.hooks.detail?.summary),
    );
  }
}
...
connectDB("APP").then(() => {
  ...
  console.log(
    `🦊 Elysia is Att running at ${app.server?.hostname}:${port} ${moment().format(
